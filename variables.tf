...
variable "sql_mastodon_user_password" {
  description = "Initial password to use for the mastodon user in the PSQL DB. It is recommended to set this in env vars, e.g. `export TF_VAR_sql_mastodon_user_password=<token>`"
  type        = string
}
...
variavle "num_sidekiq_replicas" {
  deescription = "Number of sidekiq pod replicas to create"
  type         = number
  default      = 1
}
